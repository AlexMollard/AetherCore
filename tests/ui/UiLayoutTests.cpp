...
TEST_CASE("ResolveRect clamps crossed offsets to non-negative size")
{
	UIRect r;
	r.anchorMin = r.anchorMax = {0.5f, 0.5f};
	r.offsetMin = {50.f, 40.f};
	r.offsetMax = {-50.f, -40.f};

	const glm::vec4 out = ResolveRect({0, 0, 200, 100}, r);
	CHECK(out.x == doctest::Approx(150.f));
	CHECK(out.y == doctest::Approx(90.f));
	CHECK(out.z == doctest::Approx(0.f));
	CHECK(out.w == doctest::Approx(0.f));
}
