...
/// <summary>
/// A slab that sinks into the floor while its <see cref="Channel"/> is held down by a
/// <see cref="PressurePlate"/>, and grinds back up when the weight comes off.
///
...
public sealed class Gate : EntityScript
{
    ...
    public override void OnAttach()
    {
        ...
        _t = PressurePlate.IsHeld(Channel) ? 1.0f : 0.0f;
        Apply();
    }

    public override void OnUpdate(float deltaTime)
    {
        float target = PressurePlate.IsHeld(Channel) ? 1.0f : 0.0f;
