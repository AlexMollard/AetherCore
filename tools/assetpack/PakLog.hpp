...
inline void SaveLog(
        const std::filesystem::path& logPath, const std::filesystem::path& sourceDir, const std::filesystem::path& pakPath, int compressionLevel, const std::vector<LogEntry>& entries, uint64_t totalRawBytes, uint64_t pakBytes, double elapsedSecs)
{
	...
	out << std::left << std::setw(kNameW) << "  File" << std::right << std::setw(kSizeW) << "Raw" << std::setw(kSizeW) << "Pak" << std::setw(kSavingsW) << "Savings"
	    << "  " << "Hash"
	    << "\n"
	    << rule << "\n";
	...
	for (const auto& e: entries)
	{
		...
		const double ratio = (compressed && e.rawSize > 0) ? 100.0 * (1.0 - static_cast<double>(e.onDiskSize) / static_cast<double>(e.rawSize)) : 0.0;
		...
		out << std::left << std::setw(kNameW) << ("  " + e.virtualPath) << std::right << std::setw(kSizeW) << FormatSize(e.rawSize) << std::setw(kSizeW) << FormatSize(e.onDiskSize) << std::setw(kSavingsW) << savingsStr.str() << "  " << hashStr.str()
		    << "\n";
	}
	...
	out << std::left << std::setw(kNameW) << "  TOTAL" << std::right << std::setw(kSizeW) << FormatSize(totalRawBytes) << std::setw(kSizeW) << FormatSize(pakBytes) << std::setw(kSavingsW) << totalSavings.str() << "\n";
	out << "\n";
