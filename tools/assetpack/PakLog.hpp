...
namespace aether::assetpipeline
{
	        ...
	        const std::filesystem::path& logPath, const std::filesystem::path& sourceDir, const std::filesystem::path& pakPath, int compressionLevel, const std::vector<LogEntry>& entries, uint64_t totalRawBytes, uint64_t pakBytes, double elapsedSecs)
	{
		...
		for (const auto& e: entries)
		{
			...
			out << std::left << std::setw(kNameW) << ("  " + e.virtualPath) << std::right << std::setw(kSizeW) << FormatSize(e.rawSize) << std::setw(kSizeW) << FormatSize(e.onDiskSize) << std::setw(kSavingsW) << savingsStr.str() << "  "
			    << hashStr.str() << "\n";
		}
