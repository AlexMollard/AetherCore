...
namespace aether
{
	      ...
	      : m_window(config.appName, config.width, config.height), m_vulkanContext(m_window, config.appName)
	{
		...
		{
			shadowConstants.Initialize(m_vulkanContext);
		}
		...
		{
			shadowQueue.Initialize(m_vulkanContext.GetDevice().device, m_vulkanContext.GetAllocator());
		}
		...
		m_shadowPipeline = GraphicsPipeline::Create(m_vulkanContext.GetDevice().device,
		        {
	...
	void AetherCore::RegisterPasses()
	{
		...
		// Pass 2-7: CSM directional shadow cull + depth render per cascade.
		for (std::uint32_t cascade = 0; cascade < kShadowCascadeCount; ++cascade)
		{
			m_shadowDepth[cascade] = m_renderGraph.CreateTransientDepth(m_swapchain.GetDepthFormat(), m_shadowMapExtents[cascade], VK_IMAGE_USAGE_SAMPLED_BIT);
			m_shadowMapSlots[cascade] = m_renderGraph.EnsureBindlessSampled(m_shadowDepth[cascade], m_bindlessManager, m_vulkanContext.GetDevice().device);

			const std::string idx = std::to_string(cascade);
			m_renderGraph.AddComputePass("$CullDraws_Shadow_C" + idx)
			        .ExecuteCompute(
			                [this, cascade](PassContext& ctx)
			                {
				                ...
				                m_shadowRenderQueues[cascade].PrepareAndDispatch(ctx.recorder.GetCommandBuffer(), shadowFrameAddr, m_cullPass.GetPipeline(), m_cullPass.GetPipelineLayout(), ctx.frameIndex);
			                });

			m_renderGraph.AddPass("$DirectionalShadow_C" + idx)
		...
		m_forwardPass.RegisterPass(
		        ...
		        std::span<const RGImage>(m_shadowDepth.data(), m_shadowDepth.size()));
	...
	RenderFramePacket AetherCore::PrepareFrame(std::uint32_t drawSlot, std::uint64_t frameIndex)
	{
		...
		// Separate queues so each CSM cascade can do GPU cull + indirect draw
		// in its own light-space view.
		for (auto& shadowQueue: m_shadowRenderQueues)
		{
			shadowQueue.SetWriteSlot(drawSlot);
			m_scene.FlushToQueue(shadowQueue);
			m_world.FlushToQueue(shadowQueue);
		}
	...
	void AetherCore::EndFrame(const RenderFramePacket& packet)
	{
		...
		if (m_swapchain.IsFrameValid())
		{
			...
			// Build cascaded directional shadow frames (CSM), one GPU-driven cull
			// + draw path per cascade.
			...
			const Camera* mainCamForShadows = m_cameraManager.TryGetMainCamera();
	...
	std::vector<Entity> AetherCore::SpawnModel(LoadedModel& model, GraphicsPipeline& pipeline, float scale)
	{
		...
		if (model.animationDb.IsValid())
		{
			...
			for (auto& shadowQueue: m_shadowRenderQueues)
			{
				shadowQueue.SetAnimationDatabase(&model.animationDb);
			}
