...
namespace meow
{
	...
	bool Input::IsMouseButtonDown(MouseButton btn) const
	{
		...
		if (b < 0 || b >= kMaxMouseButtons) return false;
	...
	glm::vec2 Input::GetMouseDelta() const  { return m_mousePos - m_prevMousePos; }
	glm::vec2 Input::GetScrollDelta() const { return m_scrollDelta; }
