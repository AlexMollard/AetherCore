...
namespace aether
{
	...
	RenderGraph::PassBuilder& RenderGraph::PassBuilder::WriteColor(
		RGImage image,
	...
	RenderGraph::PassBuilder& RenderGraph::PassBuilder::WriteDepth(
		RGImage image,
		VkAttachmentLoadOp loadOp,
		VkAttachmentStoreOp storeOp,
		VkClearValue clearValue)
	...
	void RenderGraph::RemovePass(const std::string& name)
	{
		const auto it = std::find_if(m_passes.begin(), m_passes.end(),
			[&](const PassRecord& p) { return p.name == name; });
		if (it != m_passes.end())
		{
			m_passes.erase(it);
			m_dirty = true;
	...
	void RenderGraph::Compile()
	{
		...
		auto passWrites = [&](std::size_t idx, uint32_t resId) -> bool
			{
				...
				{
					if (a.image.id == resId) { return true; }
				}
		...
		for (std::size_t i = 0; i < N; ++i)
		{
			for (std::size_t j = i + 1; j < N; ++j)
			{
				...
				{
					if (passAccesses(j, a.image.id)) { dependent = true; break; }
				}
				if (!dependent && m_passes[i].depthWrite.has_value())
				{
					if (passAccesses(j, m_passes[i].depthWrite->image.id)) { dependent = true; }
				}
				if (dependent)
				{
					adj[i].push_back(j);
		...
		for (const std::size_t idx : sortedIndices)
		{
			...
			for (const RGImage& r : pass.textureReads)
			{
				...
				const VkPipelineStageFlags2 srcStage = (it != states.end())
					? it->second.writeStage : VK_PIPELINE_STAGE_2_TOP_OF_PIPE_BIT;
				const VkAccessFlags2 srcAccess = (it != states.end())
					? it->second.writeAccess : VK_ACCESS_2_NONE;
				const VkImageLayout oldLayout = (it != states.end())
	...
	{
		if (resourceId == kSwapchainColorId) { return target.colorImage; }
		if (resourceId == kSwapchainDepthId) { return target.depthImage; }
		const uint32_t idx = resourceId - kFirstExternalId;
		if (idx < m_externalImages.size()) { return m_externalImages[idx].image; }
		return VK_NULL_HANDLE;
	}

	VkImageView RenderGraph::ResolveView(uint32_t resourceId, const FrameTarget& target) const
	{
		if (resourceId == kSwapchainColorId) { return target.colorView; }
		if (resourceId == kSwapchainDepthId) { return target.depthView; }
		const uint32_t idx = resourceId - kFirstExternalId;
		if (idx < m_externalImages.size()) { return m_externalImages[idx].view; }
		return VK_NULL_HANDLE;
	}

	VkImageAspectFlags RenderGraph::ResolveAspect(uint32_t resourceId) const
	{
		if (resourceId == kSwapchainDepthId) { return VK_IMAGE_ASPECT_DEPTH_BIT; }
		const uint32_t idx = resourceId - kFirstExternalId;
		if (idx < m_externalImages.size()) { return m_externalImages[idx].aspect; }
