...
namespace aether
{
	namespace
	{
		...
		void EndAndSubmitOneTimeBuffer(
			VkDevice        device,
			VkCommandPool   pool,
			VkQueue         queue,
			VkCommandBuffer cmd)
		...
		BindlessManager& bindless)
	{
		...
		// Resolve via the VFS so callers can use virtual paths like "assets://textures/..."
		...
		if (pixels == nullptr)
		{
			throw std::runtime_error(
				"Texture::LoadFromFile: failed to load '" + std::string(path) + "': " +
				stbi_failure_reason());
		}
