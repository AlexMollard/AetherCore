...
namespace aether
{
	...
	void ImguiViewportRenderer::Shutdown()
	{
		...
		if (m_samplerPool) vkDestroyDescriptorPool(m_device, m_samplerPool, nullptr);
		if (m_texSetLayout) vkDestroyDescriptorSetLayout(m_device, m_texSetLayout, nullptr);
		if (m_samplerSetLayout) vkDestroyDescriptorSetLayout(m_device, m_samplerSetLayout, nullptr);
	...
	void ImguiViewportRenderer::RenderOne(PerViewport& vp, const ImDrawData& draw)
	{
		...
		{
			// Window resized / out-of-date: rebuild and present next frame.
		...
		vkCmdPipelineBarrier(fd.CommandBuffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, 0, 0, nullptr, 0, nullptr, 1, &toColor);
	...
	void ImguiViewportRenderer::DestroyOne(PerViewport& vp)
	{
		...
		{
			if (rb.vtx) vkDestroyBuffer(m_device, rb.vtx, nullptr);
			if (rb.vtxMem) vkFreeMemory(m_device, rb.vtxMem, nullptr);
			if (rb.idx) vkDestroyBuffer(m_device, rb.idx, nullptr);
			if (rb.idxMem) vkFreeMemory(m_device, rb.idxMem, nullptr);
		}
		vp.ring.clear();
