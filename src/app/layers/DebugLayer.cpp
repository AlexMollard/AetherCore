...
namespace aether::app
{
	namespace
	{
		...
		{
			if (fps >= 55.f) return { 0.40f, 0.72f, 0.46f, 1.f };
			if (fps >= 30.f) return { 0.86f, 0.71f, 0.30f, 1.f };
			return { 0.80f, 0.33f, 0.30f, 1.f };
		}

		glm::vec4 MsColor(float ms) noexcept
		{
			if (ms <= 16.667f) return { 0.40f, 0.72f, 0.46f, 1.f };
			if (ms <= 25.f)    return { 0.86f, 0.71f, 0.30f, 1.f };
			return { 0.80f, 0.33f, 0.30f, 1.f };
		}

		ImVec4 ToImVec4(glm::vec4 c) { return { c.r, c.g, c.b, c.a }; }
	...
	{
		if (m_frameHistoryCount == 0) return 0.f;
		float total = 0.f;
	...
	void DebugLayer::DrawFrameTimeGraph() const
	{
		...
		for (std::size_t i = 0; i < m_frameHistoryCount; ++i)
		{
			const std::size_t idx =
				(m_frameHistoryHead + kFrameHistorySize - m_frameHistoryCount + i) % kFrameHistorySize;
		...
		ImGui::PlotHistogram("##ft",
			ordered.data(),
			static_cast<int>(m_frameHistoryCount),
			0,
			overlay.data(),
			0.f, 33.333f,
			ImVec2(ImGui::GetContentRegionAvail().x, 72.f));
	}
	...
	void DebugLayer::OnGui(LayerContext& context)
	{
		...
		ImGui::SetNextWindowPos(
			ImVec2(io.DisplaySize.x - 12.f, 12.f),
			ImGuiCond_Once,
			ImVec2(1.f, 0.f));
		ImGui::SetNextWindowSize(ImVec2(420.f, 0.f), ImGuiCond_Once);

		ImGui::Begin("DEBUG OVERLAY", &m_visible,
		...
		ImGui::Text("Frame");    ImGui::NextColumn(); ImGui::TextUnformatted(buf.data()); ImGui::NextColumn();

		std::snprintf(buf.data(), buf.size(), "%.1f", curFps);
		ImGui::Text("FPS");      ImGui::NextColumn();
		ImGui::TextColored(ToImVec4(FpsColor(curFps)), "%s", buf.data()); ImGui::NextColumn();

		std::snprintf(buf.data(), buf.size(), "%.2f ms", curMs);
		ImGui::Text("Delta");    ImGui::NextColumn();
		ImGui::TextColored(ToImVec4(MsColor(curMs)), "%s", buf.data()); ImGui::NextColumn();

		std::snprintf(buf.data(), buf.size(), "%.1f", avgFps);
		ImGui::Text("Avg FPS");  ImGui::NextColumn();
		ImGui::TextColored(ToImVec4(FpsColor(avgFps)), "%s", buf.data()); ImGui::NextColumn();

		std::snprintf(buf.data(), buf.size(), "%.2f ms", minMs);
		ImGui::Text("Min");      ImGui::NextColumn();
		ImGui::TextColored(ToImVec4({ 0.40f, 0.72f, 0.46f, 1.f }), "%s", buf.data()); ImGui::NextColumn();

		std::snprintf(buf.data(), buf.size(), "%.2f ms", maxMs);
		ImGui::Text("Max");      ImGui::NextColumn();
		ImGui::TextColored(ToImVec4(MsColor(maxMs)), "%s", buf.data()); ImGui::NextColumn();
		...
		ImGui::Text("Tonemap");  ImGui::NextColumn();
		...
		ImGui::Text("FXAA");    ImGui::NextColumn();
		ImGui::TextColored(fxaa ? ImVec4(0.4f,0.72f,0.46f,1.f) : ImVec4(0.5f,0.6f,0.69f,1.f),
			fxaa ? "On" : "Off"); ImGui::NextColumn();
		...
		ImGui::TextUnformatted(buf.data()); ImGui::NextColumn();
		...
		if (cam)
		{
			...
			ImGui::Text("Position"); ImGui::NextColumn(); ImGui::TextUnformatted(buf.data()); ImGui::NextColumn();

			std::snprintf(buf.data(), buf.size(), "%.0f deg", cam->GetFovDegrees());
			ImGui::Text("FOV");      ImGui::NextColumn(); ImGui::TextUnformatted(buf.data()); ImGui::NextColumn();

			std::snprintf(buf.data(), buf.size(), "%.2f", cam->GetNearPlane());
			ImGui::Text("Near");     ImGui::NextColumn(); ImGui::TextUnformatted(buf.data()); ImGui::NextColumn();

			std::snprintf(buf.data(), buf.size(), "%.0f", cam->GetFarPlane());
			ImGui::Text("Far");      ImGui::NextColumn(); ImGui::TextUnformatted(buf.data()); ImGui::NextColumn();
			ImGui::Columns(1);
		}
		else
		{
		...
		if (context.renderer)
		{
			...
			std::snprintf(buf.data(), buf.size(), "%zu",
				context.renderer->GetPointLights().size());
			ImGui::Text("Point Lights"); ImGui::NextColumn(); ImGui::TextUnformatted(buf.data()); ImGui::NextColumn();

			std::snprintf(buf.data(), buf.size(), "%zu",
				context.renderer->GetSpotLights().size());
			ImGui::Text("Spot Lights");  ImGui::NextColumn(); ImGui::TextUnformatted(buf.data()); ImGui::NextColumn();

			std::snprintf(buf.data(), buf.size(), "%.2f",
				context.renderer->GetDirectionalLightIntensity());
			ImGui::Text("Sun Intensity"); ImGui::NextColumn(); ImGui::TextUnformatted(buf.data()); ImGui::NextColumn();
