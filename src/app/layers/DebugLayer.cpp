...
namespace aether::app
{
	...
	void DebugLayer::DrawFrameTimeGraph(aether::UIRenderer& ui, float x, float y, float width, float height) const
	{
		...
		for (std::size_t i = 0; i < m_frameHistoryCount; ++i)
		{
			...
			const glm::vec4 barColor = frameMs <= 16.667f ? glm::vec4(0.42f, 0.78f, 0.52f, 0.95f) : (frameMs <= 25.0f ? glm::vec4(0.91f, 0.74f, 0.33f, 0.95f) : glm::vec4(0.86f, 0.38f, 0.34f, 0.95f));
	...
	void DebugLayer::OnGui(LayerContext& context)
	{
		...
		context.ui->DrawText("Debug Overlay",
		        aether::UiPoint{
		                .anchor = {    1.0f,  0.0f },
		...
		const float currentFrameMs = m_frameHistoryCount > 0 ? m_frameTimesMs[(m_frameHistoryHead + kFrameHistorySize - 1) % kFrameHistorySize] : static_cast<float>(context.deltaTimeSeconds * 1000.0);
