	...
	namespace
	{
		...
		void AppendEntityRecords(SceneDescription& scene, World& world, const std::vector<Entity>& order, const std::unordered_map<std::uint32_t, int>& indexOf, const MaterialRegistry& materials, const TextureRegistry& textures)
		{
			...
			for (const Entity e: order)
			{
				...
				// Sprite Renderer / Sprite Animator / Mesh Renderer capture through the
				// serde table (below).
				...
				// merged records) capture through the serde table, each registered next to
				// its component instead of inlined here.
				SceneCaptureContext capCtx{world, e, materials, textures, indexOf, rec};
				RunCaptureSerdes(capCtx);
				// Pure data-only components, captured generically from the reflection
				// registry (every ComponentType flagged genericSerialize). Only reflected
				// (authored) fields are stored, so runtime state - baseCaptured/time/base
				// etc. - is naturally left out and resurrected as defaults on apply (what
				// the old per-type "clean" did).
				...
				// Point/Spot lights, Day Night, Tile Map, Orbit Camera and Particle Emitter
				// are pure reflected data - captured generically in the rec.reflected loop
				// above. Camera and Scripts capture through the serde table (below).
