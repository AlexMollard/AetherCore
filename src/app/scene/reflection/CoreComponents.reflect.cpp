...
AE_COMPONENT(TransformComponent, "Transform", "Core", ICON_FA_UP_DOWN_LEFT_RIGHT)
	...
	AE_FIELD_CUSTOM("position", Vec3,
	        ([](const void* c) { glm::vec3 p; glm::vec3 e; glm::vec3 s; DecomposeTRS(static_cast<const TransformComponent*>(c)->localToWorld, p, e, s); return reflect::MakeValue(p); }),
	        ([](void* c, const FieldValue& v) { auto* t = static_cast<TransformComponent*>(c); glm::vec3 p; glm::vec3 e; glm::vec3 s; DecomposeTRS(t->localToWorld, p, e, s); t->localToWorld = ComposeTransform(glm::vec3(v.vec), e, s); }))
	AE_FIELD_CUSTOM("euler", Vec3,
	        ([](const void* c) { glm::vec3 p; glm::vec3 e; glm::vec3 s; DecomposeTRS(static_cast<const TransformComponent*>(c)->localToWorld, p, e, s); return reflect::MakeValue(e); }),
	        ([](void* c, const FieldValue& v) { auto* t = static_cast<TransformComponent*>(c); glm::vec3 p; glm::vec3 e; glm::vec3 s; DecomposeTRS(t->localToWorld, p, e, s); t->localToWorld = ComposeTransform(p, glm::vec3(v.vec), s); }))
	AE_FIELD_CUSTOM("scale", Vec3,
	        ([](const void* c) { glm::vec3 p; glm::vec3 e; glm::vec3 s; DecomposeTRS(static_cast<const TransformComponent*>(c)->localToWorld, p, e, s); return reflect::MakeValue(s); }),
	        ([](void* c, const FieldValue& v) { auto* t = static_cast<TransformComponent*>(c); glm::vec3 p; glm::vec3 e; glm::vec3 s; DecomposeTRS(t->localToWorld, p, e, s); t->localToWorld = ComposeTransform(p, e, glm::vec3(v.vec)); }))
AE_COMPONENT_END()
