...
namespace
{
	...
	// Query results survive until the next overlap call, mirroring the 3D cache.
...
{
	AddBody(id, aether::Collider2DComponent{.shape = aether::Collider2DShape::Box, .size = ToGlm(size)}, bodyType);
}

AE_SCRIPT_API void aether_physics2d_add_circle(std::uint32_t id, float radius, std::int32_t bodyType)
{
	AddBody(id, aether::Collider2DComponent{.shape = aether::Collider2DShape::Circle, .radius = radius}, bodyType);
}

AE_SCRIPT_API void aether_physics2d_add_capsule(std::uint32_t id, float radius, float height, std::int32_t bodyType)
{
	AddBody(id, aether::Collider2DComponent{.shape = aether::Collider2DShape::Capsule, .radius = radius, .capsuleHeight = height}, bodyType);
}
