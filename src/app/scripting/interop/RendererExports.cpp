...
{
	ActiveContext().renderer->SetAmbientLight(ToGlm(color));
}
...
{
	ActiveContext().renderer->SetPointLightPosition(static_cast<std::uint32_t>(idx), ToGlm(pos));
}

AE_SCRIPT_API void aether_render_set_point_light_color(std::int32_t idx, Vec3 color)
{
	ActiveContext().renderer->SetPointLightColor(static_cast<std::uint32_t>(idx), ToGlm(color));
}
...
{
	return static_cast<std::int32_t>(ActiveContext().renderer->GetPointLights().size());
}

AE_SCRIPT_API void aether_render_set_spot_light_position(std::int32_t idx, Vec3 pos)
{
	ActiveContext().renderer->SetSpotLightPosition(static_cast<std::uint32_t>(idx), ToGlm(pos));
}

AE_SCRIPT_API void aether_render_set_spot_light_color(std::int32_t idx, Vec3 color)
{
	ActiveContext().renderer->SetSpotLightColor(static_cast<std::uint32_t>(idx), ToGlm(color));
}
...
{
	return static_cast<std::int32_t>(ActiveContext().renderer->GetSpotLights().size());
}
...
{
	ActiveContext().renderer->SetSkyGradient(ToGlm(horizon), ToGlm(zenith));
}
