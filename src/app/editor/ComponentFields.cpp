...
namespace aether::editor
{
	namespace
	{
		...
		json Arr3(const glm::vec3& v)
		{
			return json::array({v.x, v.y, v.z});
		}

		json Arr4(const glm::vec4& v)
		{
		...
		{
			if (w.TryGet<MaterialComponent>(e) == nullptr)
			{
				return false;
			}
			MaterialAsset a{};
			if (!DescribeMaterial(w, e, services, a))
			{
				return false;
			}
		...
		std::vector<std::string> WriteMaterial(World& w, Entity e, const json& v, ServiceContainer& services)
		{
			...
			{
				MaterialAsset seed{};
				if (!assets->GetMaterialRegistry().TryDescribe(mc->handle, seed))
				{
					return applied;
				}
			...
			if (v.contains("baseColor"))
			{
				a.baseColorFactor = JVec4(v["baseColor"], a.baseColorFactor);
				applied.emplace_back("baseColor");
			}
			if (v.contains("metallic"))
			{
				a.metallicFactor = v["metallic"].get<float>();
				applied.emplace_back("metallic");
			}
			if (v.contains("roughness"))
			{
				a.roughnessFactor = v["roughness"].get<float>();
				applied.emplace_back("roughness");
			}
			if (v.contains("occlusion"))
			{
				a.occlusionStrength = v["occlusion"].get<float>();
				applied.emplace_back("occlusion");
			}
			if (v.contains("emissive"))
			{
				a.emissiveFactor = JVec3(v["emissive"], a.emissiveFactor);
				applied.emplace_back("emissive");
			}
			if (v.contains("doubleSided"))
			{
				a.doubleSided = v["doubleSided"].get<bool>();
				applied.emplace_back("doubleSided");
			}
			if (v.contains("alphaBlend"))
			{
				a.alphaBlend = v["alphaBlend"].get<bool>();
				applied.emplace_back("alphaBlend");
			}
			if (v.contains("alphaMask"))
			{
				a.alphaMask = v["alphaMask"].get<bool>();
				applied.emplace_back("alphaMask");
			}
			if (v.contains("alphaCutoff"))
			{
				a.alphaCutoff = v["alphaCutoff"].get<float>();
				applied.emplace_back("alphaCutoff");
			}
			if (v.contains("receiveShadows"))
			{
