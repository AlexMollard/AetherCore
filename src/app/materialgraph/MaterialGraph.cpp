...
namespace aether::editor
{
	...
	std::string GenerateMaterialShader(const MaterialGraph& graph, std::string& error)
	{
		...
		// scene looking like the materials around it, fog included. Still missing next to the
		// standard shader: the occlusion texture and screen-space GTAO, which need an
		// occlusion input the graph's Output node does not carry yet.
...
{}
    ...
    float3 finalColor = direct + ambientDiffuse + ambientSpecular + emissive;
