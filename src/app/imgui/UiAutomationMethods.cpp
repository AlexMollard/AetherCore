...
namespace aether::editor
{
	...
	void AppendUiAutomationMethods(std::vector<ControlMethod>& methods)
	{
		methods.push_back({"ui.query",
		        "ui_query",
		        "List on-screen ImGui widgets from the last frame: window, label, screen rect, and hovered/active state. Optional 'window'/'label' substring filters. Use this to navigate the UI before ui_click.",
		        false,
		        ...
		        [](const json& params, MethodContext&) -> json
		        {
			        ...
			        for (const app::UiItem& item: app::UiAutomation::Get().Snapshot())
			        {
				        ...
				        items.push_back(json{{"window", item.window}, {"label", item.label}, {"x", item.x}, {"y", item.y}, {"w", item.w}, {"h", item.h}, {"hovered", g != nullptr && g->HoveredId == item.id}, {"active", g != nullptr && g->ActiveId == item.id}});
			        }
